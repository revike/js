...
let class_products = document.querySelector('.products');
...
function showCategory(category) {
    class_products.innerHTML = getProductMarkup(category);
}
...
function getProductMarkup(product) {
    ...
    for (let i = 0; i < products[product].length; i++) {
        res += `<div class="product"><div>${products[product][i].name}</div><img src="${products[product][i].imageUrl}"\
        alt=""><div>${products[product][i].price}</div><a href="https://example.com/producs/${products[product][i].id}">\
        Подробнее</a></div>`;
    }
    return res;
}
